  ...
  <div ref="rootRef" class="lecture-player">
    ...
    <Transition name="lp-fade">
      <div v-if="!isFloating || isExpanded" class="player-card" :class="{ 'is-expanded-overlay': isFloating && isExpanded }">
        ...
        <!-- Progress (pointer events for drag + click) -->
        <div
          ref="progressRef"
          class="progress-area"
    ...
    <Transition name="lp-float">
      ...
      >
        ...
        <div class="float-info">
          ...
          <div class="float-bar">
            <div class="float-progress" :style="{ width: progressPercent + '%' }" />
          </div>
        </div>
...
function setupObserver() {
  ...
  observer = new IntersectionObserver(
    ([entry]) => {
      // Khi player ra khỏi viewport (hoặc chỉ còn 1 phần nhỏ) → float
...
.progress-area {
  ...
  height: 22px;
  cursor: pointer;
  touch-action: none; /* Quan trọng: ngăn scroll khi drag trên mobile */
  margin-bottom: 6px;
}
...
.progress-handle {
  ...
  border: 3px solid var(--vp-c-brand-1, #89b4fa);
  border-radius: 50%;
...
.lp-fade-leave-active { transition: opacity 0.25s ease, transform 0.25s ease; }
.lp-fade-enter-from,
.lp-fade-leave-to { opacity: 0; transform: scale(0.98); }

.lp-float-enter-active { transition: all 0.35s cubic-bezier(0.22, 1, 0.36, 1); }
.lp-float-leave-active { transition: all 0.2s ease-in; }
.lp-float-enter-from,
.lp-float-leave-to { opacity: 0; transform: translateY(20px) scale(0.9); }
