...
<LecturePlayer
  src="/audio/stage-00/0.1.3-execution-conceptual.aac"
  title="Bài 0.1.3 — JavaScript Execution"
  subtitle="Source Code → Engine → Execution • 23 phút"
/>
...
Engine đọc dòng này, không hiểu `= ;` là gì. Lỗi xảy ra ngay ở **Parse Phase** — engine chưa chạy một dòng code nào cả.
...
Dòng 1 parse OK. Dòng 2 cũng parse OK (cú pháp đúng). Nhưng khi engine **execute** dòng 2, nó phát hiện `null` không có method `toUpperCase`. Lỗi xảy ra ở **Execution Phase**.
...
- Người nghe hiểu tại sao `try/catch` không bắt được syntax error.
...
- Nếu họ hỏi "vậy `try/catch` bao quanh cả file thì sao?" — bạn giải thích được không?
:::
