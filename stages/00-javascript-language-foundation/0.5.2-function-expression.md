...
Giải thích:
`transform` là một function được truyền vào `process`. Bên trong `process`, ta gọi `transform(value)` và trả về kết quả.
...
Giải thích:
`calculator[operation]` cho phép truy cập method dựa trên giá trị của biến operation.
